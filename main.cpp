...
#if defined(NDEBUG)
# error "Buncoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xf35fd33299a58a9bc108830732989310d518998b87ca9c41635d79af25700751");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Buncoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Buncoin Signed Message:\n";
...
{
    // Buncoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 10000); // Buncoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Buncoin: 3.5 days
static const int64 nTargetSpacing = 5 * 60; // Buncoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Buncoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Buncoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x2144a9dcc7aeb5b57ed5e575af80fd3e5f18838494ddb4b9a57da26557a53634");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Doug Jones defeates Roy Moore in Alabama senate election";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 20 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("045898b7ca0672192bf5c7c8bd95f61930937f291a20f73c26808dd088e7789a50e27dad7b9ee32b5ebdde222e114422cbce209ff953f1fe22940263c73048c892") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1527275443;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 388524576;

        if (fTestNet)
        {
            block.nTime    = 1527275384
;
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0xa25d6c5656fcb016efff7de1ea2a6724797b1dbecec8d8b472fb91679167eb3f"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfa, 0xc3, 0xba, 0xd3 }; // Buncoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BuncoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BuncoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BuncoinMiner(CWallet *pwallet)
{
    printf("BuncoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("buncoin-miner");
    ...
    try { loop {
        ...
        printf("Running BuncoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BuncoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BuncoinMiner, pwallet));
}
